...
contract Arbitrage is IFlashLoanRecipient, Ownable, ReentrancyGuard {
    // Balancer Vault per flash loan (Polygon)
    IVault private constant vault = IVault(0xBA12222222228d8Ba445958a75a0704d566BF2C8);
    
    // Router dei DEX (Polygon)
    ...
    event NetworkInfo(string network, uint256 chainId);
    ...
    constructor(
        ...
        // Emetti evento con info rete
        emit NetworkInfo("Polygon", block.chainid);
    }
    ...
    /**
     * @notice Esegue uno swap su un DEX specificato (ottimizzato per Polygon)
     */
    function _swapOnDEX(
        ...
        // Esegui lo swap con deadline più lunga per Polygon
        uint256[] memory amounts = router.swapExactTokensForTokens(
            amountIn,
            0, // accepta qualsiasi quantità di token out
            path,
            address(this),
            block.timestamp + 600 // 10 minuti di deadline (più lungo per L2)
        );
    ...
    /**
     * @notice Ottiene informazioni sulla rete corrente
     */
    function getNetworkInfo() external view returns (string memory network, uint256 chainId) {
        if (block.chainid == 80002) {
            return ("Amoy Testnet", block.chainid);
        } else if (block.chainid == 80001) {
            return ("Mumbai Testnet (Deprecated)", block.chainid);
        } else if (block.chainid == 137) {
            return ("Polygon Mainnet", block.chainid);
        } else {
            return ("Unknown", block.chainid);
        }
    }
} 