...
describe('Arbitrage Contract', function() {
    ...
    beforeEach(async function() {
        ...
        mockToken = await MockToken.deploy('Mock Token', 'MOCK', 18, ethers.utils.parseEther('1000000'));
    ...
    describe('Deployment', function() {
        ...
        it('Should set correct router address', async function() {
            console.log('🧪 Test: verifica indirizzo router PancakeSwap');
            
            const pancakeAddr = await arbitrageContract.pancakeRouter();
            
            expect(pancakeAddr).to.equal('0xD99D1c33F9fC3444f8101754aBC46c52416550D1');
            
            console.log('✅ Router verificato:', {
                pancake: pancakeAddr
            });
        });
    });
    ...
    describe('Access Control', function() {
        it('Should only allow owner to execute triangular arbitrage', async function() {
            console.log('🧪 Test: controllo accesso esecuzione arbitraggio triangolare');
            ...
            await expect(
                arbitrageContract.connect(addr1).executeTriangularArbitrage(pathIndex, amountIn)
        ...
        it('Should have correct token addresses', async function() {
            console.log('🧪 Test: verifica indirizzi token');
            
            const wbnbAddr = await arbitrageContract.WBNB();
            const busdAddr = await arbitrageContract.BUSD();
            const usdtAddr = await arbitrageContract.USDT();
            const usdcAddr = await arbitrageContract.USDC();
            
            expect(wbnbAddr).to.equal('0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd');
            expect(busdAddr).to.equal('0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee');
            expect(usdtAddr).to.equal('0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684');
            expect(usdcAddr).to.equal('0x64544969ed7EBf5f083679233325356EbE738930');
            
            console.log('✅ Token verificati:', {
                wbnb: wbnbAddr,
                busd: busdAddr,
                usdt: usdtAddr,
                usdc: usdcAddr
        ...
        it('Should have correct triangular paths', async function() {
            console.log('🧪 Test: verifica percorsi triangolari');
            
            const path0 = await arbitrageContract.triangularPaths(0);
            const path1 = await arbitrageContract.triangularPaths(1);
            
            expect(path0.name).to.equal('BNB-BUSD-USDT');
            expect(path1.name).to.equal('BNB-USDT-BUSD');
            
            console.log('✅ Percorsi triangolari verificati:', {
                path0: path0.name,
                path1: path1.name
            });
        });
